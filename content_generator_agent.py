...
- Video Resolution: {config.video_resolution}
- Video Aspect Ratio: {config.video_aspect_ratio}
...
4. {"Always generate a caption after creating images/videos" if config.enable_captions else "Only generate captions when explicitly asked"}
5. Optimize hashtags for each platform (Instagram, LinkedIn, Twitter, etc.)
6. Be conversational and help refine ideas through dialogue

When generating content:
- For images: Use size={config.image_size} and quality={config.image_quality}
- For videos: Use duration={config.video_duration}, resolution={config.video_resolution}, aspect_ratio={config.video_aspect_ratio}
...
def create_content_agent():
    ...
    # Using "openai:gpt-5" format as per the API docs
    agent = create_agent(
        model="openai:gpt-5",
        tools=tools,
